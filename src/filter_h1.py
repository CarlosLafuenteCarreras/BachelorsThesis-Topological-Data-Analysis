...
def toro(n_puntos=200, ruido=0.2, n_outliers=10):
...
def filtrar_por_impacto_topologico(puntos, epsilon, iteraciones):
    S = puntos.copy()
...
def main():
    ...
    # Filtramos el toro
    puntos_filtrados = filtrar_por_impacto_topologico(puntos,0.02,100)
    ...
    resultados_filtrados = ripser(puntos_filtrados,maxdim=2)
    diagramas_filtrados = resultados_filtrados['dgms']
    # Muestra diagramas de persistencia
    plot_diagrams(diagramas, show=True)
    plot_diagrams(diagramas_filtrados, show=True)
